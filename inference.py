...
def main(args):
    ...
    args.pretrained = "checkpoint/kitti2015_ck/checkpoint.tar"
    args.with_spn = True
    ...
    root_output_folder = f"output2/{phone_type}"
    ...
    for depth_folder, mode in product(depth_folders, modes):
        ...
        for _, (left_image_path, right_image_path) in enumerate(zip(left_image_paths, right_image_paths)):
            ...
            if  np.mean(error) < 1.5 and np.mean(roi_depth_mask) > 0:
            ...
            np.save(os.path.join(raw_disp_output_folder, os.path.basename(left_image_path).replace(".png", "_anynet.npy")), disp)
