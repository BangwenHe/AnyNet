...
def main(args):
    ...
    model.eval()
    model.cuda()
    phone_type = "mate40pro_test_20220718"
    # phone_type = "p30pro"
    stereo_calibration_file = f"calib_result/{phone_type}.yml"
    
    root_image_folder = "images/mate40pro_test_20220718"
    ...
    for depth_folder, mode in product(depth_folders, modes):
        ...
        raw_depth_output_folder = os.path.join(root_output_folder, depth_folder, mode.replace("processed", "depth"))
        image_folder = os.path.join(root_image_folder, depth_folder, mode)
        os.makedirs(output_folder, exist_ok=True)
        os.makedirs(raw_depth_output_folder, exist_ok=True)
        ...
        for _, (left_image_path, right_image_path) in enumerate(zip(left_image_paths, right_image_paths)):
            ...
            np.save(os.path.join(raw_depth_output_folder, os.path.basename(left_image_path).replace(".png", "_anynet.npy")), points_3d[:, :, -1])
